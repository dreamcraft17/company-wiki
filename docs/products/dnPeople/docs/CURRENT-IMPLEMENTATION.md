...
| AI helpers | HR assistant: intent router → Prisma tools (self leave/attendance/payroll; HR headcount/contracts) or FAQ/policy lexical RAG + citations; **Gemini** synthesis utama; OpenAI-compatible fallback; explicit AI disclosure; prompt-injection/PII guardrails; ASK audit; structured answer feedback + full-period admin quality analytics; DOKU-aligned payment FAQ; provider timeout/fallback | `/assistant`, `/admin/analytics/assistant` | `/assistant`, `/admin/analytics/assistant` | Available (`ai:assistant` Professional+); LLM optional |
...
| Internal Admin Console | SUPER_ADMIN SaaS panel: customers/impersonation, billing/refunds, analytics (incl. first-party **marketing page views** via `POST /public/page-views` + `/admin/analytics/web-traffic`), tickets+KB/CSAT, content CRUD, feature flags (+ runtime), health alerts/logs, audit; MFA gate; DN Tech `isPlatformOperator` tenant excluded from customer/MRR metrics | `/admin/*` | `/admin/*` | Available (PRD v15.0); live latency Conditional |
